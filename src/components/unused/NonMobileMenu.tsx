...
export default function NonMobileMenu({ menuItems }: MenuItems) {
  return (
    <div
      className={
        "hidden lg:flex justify-between items-center 2xl:text-lg 2xl:p-2"
      }
    >
      <div
        className={"flex justify-center items-center 2xl:gap-2 2xl:text-2xl"}
      >
        <h1
          className={
            "flex items-center justify-center bg-beige text-navy-blue rounded-full font-bold uppercase 2xl:size-11 pointer-events-none select-none"
          }
        >
          Q
        </h1>
        ...
        {menuItems.map((menuItem, index) => (
          <li key={`${menuItem}-${index}`}>
            <a
              ...
              className={"tracking-wider font-normal hover:text-white"}
              draggable={false}
            >
              {menuItem}
            </a>
      ...
      <a
        href={"#contact"}
        className={
          "inline-block text-navy-blue bg-silver rounded-full hover:bg-white hover:text-black 2xl:py-2 2xl:px-8"
        }
      >
        Contact Me
      </a>
