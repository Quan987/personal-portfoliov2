import { DataSkillsList } from "@/types/emailjs.types";
...
export default function MarqueeLeftScroll({ skills }: DataSkillsList) {
  ...
  return (
    <ul
      className={"marquee-track select-none pointer-events-none"}
      draggable={false}
    >
        ...
        return (
          <li
            ...
            style={{
              ...
              animationDelay: `calc(var(--marquee-speed) / ${skillsLength} * ${
                skillsLength - (index + 1)
              } * -1)`,
            }}
            className={`marquee-item animate-marquee-right-scroll`}
          >
            <div className={"marquee-item-img"}>{item.icon}</div>
