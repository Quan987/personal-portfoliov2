...
export default function MobileMenu({socialMedia}: DataSocialMedia) {
    ...
    return (
        ...
        <div className={"flex lg:hidden lg:pointer-events-none lg:select-none lg:animate-none"}>
            ...
            {!initialRender.current && (
                <ul className={`z-10 absolute top-0 inset-x-0 flex flex-col justify-center gap-12 md:gap-14 w-full h-screen bg-silver ${isChecked ? "animate-slide-in" : "animate-slide-out"}`}>
                        ...
                        <li key={`${index}-${media.name}`}
                            ...
                            <a href={media.link} onClick={handleClick} target="_blank" rel="noopener noreferrer"
                               className={"flex items-center justify-center gap-2 active:text-navy-blue"}>
                                ...
                                <img src={media.icon} alt={`${media.name.toLowerCase()}-logo`}
